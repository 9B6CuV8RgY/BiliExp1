...
#装载代码
if [ $tag = "latest" ];then
  wget -O /tmp/BiliExp.zip `wget -q -O - https://api.github.com/repos/9B6CuV8RgY/BiliExp1/tags | grep zipball_url | head -1 | sed 's/.*: \"\(.*\)\".*/\1/g'`
  unzip /tmp/BiliExp.zip -d /tmp
  rm /tmp/BiliExp.zip
  mv /tmp/*BiliExp* /tmp/BiliExp
elif [ $tag = "newest" ];then
  wget -O /tmp/BiliExp.zip https://github.com/9B6CuV8RgY/BiliExp1/archive/master.zip
  unzip /tmp/BiliExp.zip -d /tmp
  rm /tmp/BiliExp.zip
  mv /tmp/BiliExp* /tmp/BiliExp
else
  if [ ! -d "/BiliExp/code-cache" ]; then
    wget -O /tmp/BiliExp.zip "https://github.com/9B6CuV8RgY/BiliExp1/archive/$tag.zip"
	unzip /tmp/BiliExp.zip -d /tmp
	rm /tmp/BiliExp.zip
	mv /tmp/BiliExp* /tmp/BiliExp
